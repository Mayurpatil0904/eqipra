// frontend/src/components/BulkUploadEquipmentModal.tsx
//
// Lets an admin onboard an entire department's worth of equipment in
// one go via Excel/CSV, instead of filling the single-item modal
// dozens of times. Mirrors the existing Team roster upload pattern.
...
interface Props {
  onClose: () => void;
  onDone: () => void; // refresh inventory list
}

type Result = { message: string; added: any[]; skipped: { row: number; name?: string; reason: string }[] };

export function BulkUploadEquipmentModal({ onClose, onDone }: Props) {
  ...
  const handleDownloadTemplate = async () => {
    try {
      await equipmentApi.downloadBulkUploadTemplate();
    } catch (e: any) {
  ...
  const handleUpload = async () => {
    ...
    setBusy(true);
    try {
      const res = await equipmentApi.bulkUpload(file);
      setResult(res);
      if (res.added.length > 0) {
        toast.success(`${res.added.length} item(s) added to inventory.`);
        onDone();
      }
    } catch (e: any) {
    ...
    <div className="fixed inset-0 z-[300] flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm"
      ...
      <div className="bg-card rounded-2xl shadow-2xl border border-border w-full max-w-lg p-6 animate-fade-in my-4 max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex items-start justify-between mb-5">
          <div>
            <h2 className="font-display text-lg font-bold">Bulk Upload Equipment</h2>
            <p className="text-sm text-muted-foreground mt-0.5">
              Onboard an entire department's equipment at once via Excel or CSV.
            </p>
          </div>
        ...
        <div className="bg-muted/50 rounded-xl p-4 mb-4">
          ...
          <p className="text-xs text-muted-foreground mb-3">
            Includes the exact columns Equipra expects, with two filled-in example rows.
          </p>
        ...
        <button onClick={handleUpload} disabled={!file || busy}
          ...
          {busy ? <><Loader2 className="h-4 w-4 animate-spin" />Uploading…</> : <><Upload className="h-4 w-4" />Upload & Add Equipment</>}
        </button>
                ...
                <div className="max-h-32 overflow-y-auto space-y-1">
                  {result.added.map((a, i) => (
                    <div key={i} className="flex items-center justify-between px-2.5 py-1.5 rounded bg-muted/50">
                      ...
                      <span className="font-mono text-muted-foreground">{a.equipmentId}</span>
                    </div>
                  ))}
                </div>
              </div>
